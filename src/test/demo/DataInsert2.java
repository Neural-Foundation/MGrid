 ...
 */
public class DataInsert2 implements SearchListener, P2PListener {
	...
	Vector<IndexEntry> entries = new Vector<IndexEntry>();
		...
		config.set("hbase.regionserver.lease.period", "3600000");
		config.set("hbase.rpc.timeout", "3600000");
		MultiDimensionalSpec spec = new MultiDimensionalSpec(Ints.asList(17, 17));
		SpaceFillingCurve sfc = new CompactHilbertCurve(spec);
		HTable table = new HTable(config, "mgrid2dm");
	//	BitVector bv = MGridUtils.HilbertConvertor(64430L, 64764L);
		Scan scan = new Scan();
		scan.setCacheBlocks(false);
		scan.setCaching(1);
		scan.addFamily(Bytes.toBytes("data"));
		//scan.setStartRow(bv.toBigEndianByteArray());
		...
		try {
			for (Result res : scanner) {
				rowCount++;
				byte[]  row = res.getRow();
				bindex.copyFromBigEndian(row);
				sfc.indexInverse(bindex, point);
				...
				for (KeyValue kv : res.raw() ) {
					...
					count++;
					if (count % 100000 == 0) {
						System.out.println(" inserting "+x+" "+y+" "+id + " "+entry.getKey().toString());
						index.insert(entries);
						System.out.println("inserted "+count+" entries and "+rowCount+" rows");
	...
	public static void main(String[] args) throws IOException {
		...
		DataInsert2 storage = new DataInsert2();
