...
public class HelloWorld implements SearchListener, P2PListener {
	 ...
	 * 
	 * @param bootIP
	 *            address of host to bootstrap from
	 * @param bootPort
	 *            service port of the boostrapping host
	 * @param properties
	 *            additional properties needed to initialize the peer-to-peer
	 *            layer
	 */
	public void run(InetAddress bootIP, int bootPort, java.util.Properties properties) {
		/** P2P INITIALIZATION **/
		...
		/*
		 * // creating and registering data type. Type type =
		 * indexFactory.createType("SimpleType"); TypeHandler handler = new
		 * DefaultTypeHandler(type); indexFactory.registerTypeHandler(type,
		 * handler);
		 */
	...
	/**
	 * In this small demo program we want to insert some data inside the
	 * network, update some of them and perform a query and a range query.
	 */
	protected void storageDemo(Type type) throws IOException {
		...
		Long x13 = 163L;
		Long y13 = 163L;
		Long id13 = System.nanoTime();
		Point point13 = new Point(x13, y13, id13);
		IndexEntry entry13 = indexFactory.createIndexEntry(type, point13);
		entries.add(entry13);
		// insert our date entries in the system index.insert(entries);
		System.out.println("Inserted data entries."+" "+entry13.getKey().toString());
		//index.insert(entries);
		WaitingArea.waitTillSignal(1000 * 2);
		 
		// ensure that PGrid will build the network (this is optional, by
		// default P-Grid build the network)
		// you can have a look at PGrid.ini at the root of you home directory to
		// see if initExchange is set
		// to true.
	...
	/**
	 * In this demo, we want to send a custom message to an other peer to
	 * illustrate the routing facilities of the common interface.
	 */
	protected void p2pDemo() {

		String s = "Welcome to P-Grid! I'm peer "
				+ p2p.getLocalPeer().getIP().getCanonicalHostName()
...
//			BitVector[] xy = MGridUtils.HilbertInverseConvertor(Long.parseLong(entry.getKey().toString(),2));
	...
	private static int[][] generateRanges(MultiDimensionalSpec spec,
			int[] maxLengthPerDimension, Long xmin, Long ymin, Long xmax,
			Long ymax) {
		int[][] ranges = new int[spec.getBitsPerDimension().size()][2];
		// create a range query as (xmin , ymin) (xmax, ymax) = (1,1) (4,4),
		// both ends inclusive
	...
	private static List<LongRange> rangesToQueryRegion(int[][] ranges) {
		...
		for (int j = 0; j < ranges.length; ++j) {
			// scan is exclusive of range , can also set InclusiveStopFilter in
			// scan
	...
	private void hilbertRangeQuery(Long xmin, Long xmax, Long ymin, Long ymax,
			IndexFactory indexFactory, Type type, Index index,
			SearchListener listner) {
		...
		int[][] ranges = generateRanges(spec, maxLengthPerDimension, xmin,
				ymin, xmax, ymax);
		...
		for (int i = 0; i < indexRanges.size(); ++i) {
			...
			p2p.index.Query rangeQuery = indexFactory.createQuery(type,
