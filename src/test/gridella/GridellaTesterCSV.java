...
public class GridellaTesterCSV {
	 ...
	 */
	private static final GridellaTesterCSV SHARED_INSTANCE = new GridellaTesterCSV();
	 ...
	 *      Lazy instantiation - Balancing performance and resource usage</a>
	 */
	public static GridellaTesterCSV sharedInstance() {
	...
	public static void main(String[] argv) {
		...
		longOpts[13] = new LongOpt("monitoringNode", LongOpt.REQUIRED_ARGUMENT, null, 'm');
		...
		while ((opt = args.getopt()) != -1) {
			...
			case 'm':
				try {
					monitorNode = args.getOptarg();
				} catch (NumberFormatException e) {
					System.err.println("invalid value for option '-m|-monitoringNode'");
					usage();
				}
				break;
			case 't':
		...
		if ((port != -1) || (cfgFile != null) || (routeFile != null) || (repFact != -1) || (bootstrap!=null)) {
			...
			if (monitorNode!=null){
				props.setProperty(Properties.MONITORED_MODE, "true");
				props.setProperty(Properties.MONITORING_HOST, monitorNode);
			}
		}
...
//		String[] filenames = testFiles.uniformFilenames(nbFiles);
		...
		CSVIndexTable localStore = pgrid.getIndexManager().getCSVIndexTable();
		try {
			localStore.openFileForWriting();

			Collection<IndexEntry> itms = new ArrayList<IndexEntry>();
			...
			for (int i = 0; i < (int)(nbFiles * 1.0); i++) {
				...
				pgrid.IndexEntry ie = new XMLFileIndexEntry(PGridP2PFactory.sharedInstance().generateGUID(), type,
						PGridP2PFactory.sharedInstance().generateKey(rnd+parts[1]+"___"+i), pgrid.getLocalHost(), parts[1]+"___"+i/*+":"+parts[0]+":"+ parts[2]*/);

				localStore.addIndexEntry(ie);

//				itms.add(ie);
//				if(i%1000 == 0){
//					pgrid.getIndexManager().insertIndexEntries(itms, false);
//					itms.clear();
//				}
