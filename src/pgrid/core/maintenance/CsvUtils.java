...
public class CsvUtils {
	...
	/**
	 * Merges Two files and removes any duplicate entries
	 * 
	 * @param csvFile1
	 * @param csvFile2
	 */
	public static CSVIndexTable mergeCSVIndexTables(CSVIndexTable csvFile1, CSVIndexTable csvFile2){
		CSVIndexTable mergedCsvIndexTable = new CSVIndexTable("MergedBloom.csv",false);
		long time = System.currentTimeMillis();
		// Create a bloom filter "20*expSize" bits in size to contain Strings, optimized to
		// contain "expSize" items
		int expSize = csvFile1.count() + csvFile2.count();
		int cntrlExpecSize = Math.max(expSize, 1000);
		SimpleBloomFilter<String> ies = new SimpleBloomFilter<String>(32*cntrlExpecSize, cntrlExpecSize);

		try{
			csvFile1.openFileForReading();
			csvFile2.openFileForReading();
			mergedCsvIndexTable.openFileForWriting();
			String line = "";
			while((line = csvFile1.getNextLine()) != null){
				String GUID = line.split(",")[2];
				if(!ies.contains(GUID)){
					ies.add(GUID);
					mergedCsvIndexTable.addIndexEntry(line);
				}
			}
			while((line = csvFile2.getNextLine()) != null){
				String GUID = line.split(",")[2];
				if(!ies.contains(GUID)){
					ies.add(GUID);
					mergedCsvIndexTable.addIndexEntry(line);
			...
			csvFile1.closeFileOnReading();
			csvFile2.closeFileOnReading();
			mergedCsvIndexTable.closeFileForWriting();
		}
		if(expSize > mergedCsvIndexTable.count()){
			Constants.LOGGER.finest("Total Size Before MERGING : "+ expSize +"("+csvFile1.count()+" + "+csvFile2.count()+")");
			Constants.LOGGER.finest("Total Size After MERGING : "+ mergedCsvIndexTable.count());
			Constants.LOGGER.finest("MERGING TimeTaken(secs) : "+(System.currentTimeMillis()-time)/1000);
		}
		return mergedCsvIndexTable;
 ...
 * http://dark-code.bulix.org/1epygx-65873
 * 
 * @param <E>
 *            The type of object the BloomFilter should contain
 */
class SimpleBloomFilter<E> implements Set<E>, Serializable {
