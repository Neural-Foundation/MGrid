 ...
 * @version 1.0.0
 */
public class FidgetExchangeReplyMessage extends PGridMessageImp {

	/**
	 * A part of the XML string.
	 */
	public static final String XML_FIDGET_EXCHANGE_REPLY = "FidgetExchangeReply";

	/**
	 * A part of the XML string.
	 */
	public static final String XML_FIDGET_EXCHANGE_RANDOM = "RandomSeed";
	...
	/**
	 * Random seed to split the fidget list
	 */
	private double mRandom = -1;
	 ...
	 * Random generator
	 */
	static protected Random mRnd = new Random();
	...
	protected String getXMLMessageName() {
		return XML_FIDGET_EXCHANGE_REPLY;
	}

	/**
	 * Default constructor
	 */
	public FidgetExchangeReplyMessage() {
	}
	 ...
	 */
	public FidgetExchangeReplyMessage(MessageHeader header) {
		super(header);
	}

	/**
	 * Creates a new fidget exchange reply with given values for bootstrapping.
	 *
	 * @param host         the creating host.
	 * @param routingTable the Routing Table of the creating host.
	 */
	public FidgetExchangeReplyMessage(GUID guid, PGridHost host, XMLRoutingTable routingTable) {
		super(guid);
		mRoutingTable = routingTable;
		mRandom = FidgetExchangeReplyMessage.mRnd.nextDouble();
	}
	...
	public synchronized void startElement(String uri, String lName, String qName, Attributes attrs) throws SAXException {
		if (qName.equals(FidgetExchangeReplyMessage.XML_FIDGET_EXCHANGE_REPLY)) {
			String tmp = attrs.getValue(FidgetExchangeReplyMessage.XML_FIDGET_EXCHANGE_RANDOM);
			if (tmp != null)
				mRandom = Double.parseDouble(tmp);
	...
	public synchronized String toXMLString(String prefix, String newLine) {
		...
		strBuff.append(prefix).append(XML_ELEMENT_OPEN).append(FidgetExchangeReplyMessage.XML_FIDGET_EXCHANGE_REPLY); // {prefix}<FidgetExchangeReply GUID="mGUID"

		if (mRoutingTable == null) {
			strBuff.append(XML_ELEMENT_END_CLOSE).append(newLine);
		} else {
			strBuff.append(XML_SPACE).append(FidgetExchangeReplyMessage.XML_FIDGET_EXCHANGE_RANDOM).append(XML_ATTR_OPEN).append(mRandom).append(XML_ATTR_CLOSE);  // Random seed
			...
			strBuff.append(prefix).append(XML_ELEMENT_OPEN_END).append(FidgetExchangeReplyMessage.XML_FIDGET_EXCHANGE_REPLY).append(XML_ELEMENT_CLOSE).append(newLine); // {prefix}</FidgetExchangeReply>{newLine}
		}
