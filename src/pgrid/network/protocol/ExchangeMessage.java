 ...
 * @version 1.0.0
 */
public class ExchangeMessage extends PGridMessageImp implements IFileStreamingMessage{
	 ...
	 */
	public Object clone() {
		ExchangeMessage msg = (ExchangeMessage) super.clone();
		msg.mExchange = new Exchange(mExchange.getGUID(), mExchange.getRecursion(),
				...
				mExchange.getRoutingTable(), mExchange.getIndexTable());
	...
	public ExchangeMessage(GUID guid, PGridHost host, int recursion, int lCurrent, int minStorage, double replicaEst,
		...
		mSeparatedIndexEntries = separatedIndexEntries;
	}
...
//		mXMLIndexTable = new XMLIndexTable(dataItems, sign);
	...
	public synchronized void startElement(String uri, String lName, String qName, Attributes attrs) throws SAXException {
		if (qName.equals(XML_EXCHANGE)) {
			...
			if (guidStr != null)
				mExchange.setGUID(new pgrid.GUID(attrs.getValue(XML_EXCHANGE_GUID)));
...
//			mExchange.setIndexTable(new DBIndexTable(mExchange.getHost()));
	...
	public synchronized String toXMLString(String prefix, String newLine) {
		...
		strBuff.append(XML_SPACE + XML_EXCHANGE_GUID + XML_ATTR_OPEN + mExchange.getGUID().toString() + XML_ATTR_CLOSE); // _GUID="GUID"
		...
		if (mExchange.getRandomNumber() != Double.MIN_VALUE)
			strBuff.append(XML_SPACE + XML_EXCHANGE_RANDOM_NUMBER + XML_ATTR_OPEN + String.valueOf(mExchange.getRandomNumber()) + XML_ATTR_CLOSE); // _RandomNumber="RANDOM NUMBER"
		strBuff.append(XML_SPACE + XML_EXCHANGE_RECURSION + XML_ATTR_OPEN + mExchange.getRecursion() + XML_ATTR_CLOSE); // _Recursion="RECURSION"
		strBuff.append(XML_SPACE + XML_EXCHANGE_LEN_CURRENT + XML_ATTR_OPEN + mExchange.getLenCurrent() + XML_ATTR_CLOSE); // _CurrentLength="LEN_CURRENT"
		strBuff.append(XML_SPACE + XML_EXCHANGE_MINSTORAGE + XML_ATTR_OPEN + mExchange.getMinStorage() + XML_ATTR_CLOSE); // _MinStorage="MinStorage"
		strBuff.append(XML_SPACE + XML_EXCHANGE_REPLICA_ESTIMATE + XML_ATTR_OPEN + mExchange.getReplicaEstimate() + XML_ATTR_CLOSE); // _ReplicaEstimate="REPLICA_ESTIMATE"
